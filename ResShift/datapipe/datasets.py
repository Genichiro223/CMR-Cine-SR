...
def create_dataset(dataset_config):
    ...
    if dataset_config['type'] == 'cinesr':
        dataset = Cine(
                path=dataset_config['params']['dir_paths'],
...
class Cine(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            ...
            target, source = torch.chunk(image, chunks=2, dim=0)
        out_dict = {'lq':source, 'gt':target}
        return out_dict
...
class BaseDataFolder(Dataset):
    def __init__(
        ...
        file_paths_all = util_common.scan_files_from_folder(dir_path, im_exts, recursive)  # 一个包含所有文件路径的列表
        self.file_paths = file_paths_all if length is None else random.sample(file_paths_all, length)  # 如果length为None，那么就是所有文件路径的列表，否则就是从所有文件路径的列表中随机取length个
        self.file_paths_all = file_paths_all  # 所有文件路径的列表

        self.length = length
        self.need_path = need_path  # 是否需要路径
        self.dir_path_extra = dir_path_extra
        self.transform = get_transforms(transform_type, transform_kwargs)

    def __len__(self):
        return len(self.file_paths)  # 返回文件路径的个数

    def __getitem__(self, index):  # 这里的index是一个整数
        im_path = self.file_paths[index]  # 从文件路径列表中取出第index个文件路径

...
def scan_files_from_folder(dir_paths, exts, recursive=True):
    ...
    Input:
        ...
        exts: list
    '''
    exts = [exts, ] if isinstance(exts, str) else exts  # 这里是一个包含扩展名的列表
    dir_paths = [dir_paths, ] if isinstance(dir_paths, str) else dir_paths  # 包含路径名的列表
    ...
    return file_paths  # 返回一个包含所有文件路径的列表

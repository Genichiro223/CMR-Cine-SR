...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb):
        for layer in self:  # self是nn.Sequential的实例，是一个包含了多层的容器，可以通过索引来访问  
            if isinstance(layer, TimestepBlock):  # 如果layer是TimestepBlock的实例
                x = layer(x, emb)  # 调用layer的forward方法，将x和emb作为参数传入
            else:  
                x = layer(x)  # 否则只传入x
        return x
...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        self.out_channels = out_channels or channels  # 若不指定输出通道数，则同输入通道数
        self.use_conv = use_conv  # 是否使用卷积
        self.dims = dims  # 输入数据的维度
        if use_conv:
            self.conv = conv_nd(dims, self.channels, self.out_channels, 3, padding=1)  # 若使用卷积，则创建一个卷积层作为self的conv属性

    def forward(self, x):
        assert x.shape[1] == self.channels  # 判断输入数据的通道是否于在指定的输入通道数相等
        if self.dims == 3:
            x = F.interpolate(
                x, (x.shape[2], x.shape[3] * 2, x.shape[4] * 2), mode="nearest"
            )
        else:
            x = F.interpolate(x, scale_factor=2, mode="nearest")  # 通过最近邻的方法对图像插值，size变为原来两倍
        if self.use_conv:
            x = self.conv(x)
        return x
...
class ResBlock(TimestepBlock):  # 这里继承自TimestepBlock，所以forward方法中有两个参数
    """
    ...
    def __init__(
        ...
        # the "normalization()"" here is the group normalization with the number of groups 32
        # the size won't be changed
    ...
    def forward(self, x, emb):
        if self.updown:
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1]  # in_rest: normalization and activation; in_conv: convolution layer
            h = in_rest(x)  # normalize and activate the input
            h = self.h_upd(h)  # up
            x = self.x_upd(x)
            h = in_conv(h)
        else:
...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        self.in_channels = in_channels  # 模型的输入通道, 2
        self.model_channels = model_channels  # 模型的基础通道数 160
        self.out_channels = out_channels  # 模型的输出通道数
        self.attention_resolutions = attention_resolutions  # 调用attention 的分辨率，[64,32,16,8]
        self.dropout = dropout  # dropout概率
        self.channel_mult = channel_mult  # [1, 2, 2, 4]
        self.conv_resample = conv_resample  # True
        self.dtype = th.float16 if use_fp16 else th.float32
        self.num_heads = num_heads  # 默认是1
        self.num_head_channels = num_head_channels  # 默认是-1，从config文件中读出是32
        self.cond_lq = cond_lq  # True

        time_embed_dim = model_channels * 4  # 默认是160
        self.time_embed = nn.Sequential(
            linear(model_channels, time_embed_dim),
            nn.SiLU(),
            linear(time_embed_dim, time_embed_dim),
        ...
        ch = input_ch = int(channel_mult[0] * model_channels)  # ch=160, input_ch=160
        in_channels += base_chn  # after adding the base_chn, the in_channels becomes 2
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(conv_nd(dims, in_channels, ch, 3, padding=1))]
        )  # dims=2, in_channels=2, ch=160, here will return a conv2d layer, the input_channels is 2,and output channels is 160
        input_block_chans = [ch]
        ds = image_size  # ds = 128
        for level, mult in enumerate(channel_mult):  # level is a list of [0,1,2,3], mult is a list of [1,2,2,4]
            # level表示当前的U-Net的层级，mult表示当前层的通道的倍数
            for jj in range(num_res_blocks[level]):  # num_res_blocks[level] 变量表示当前层中残差块的数量，通过这个循环来组装layer
                layers = [
                    ResBlock(
                        ch,
                        time_embed_dim,
                ...
                ch = int(mult * model_channels)  # 循环过程中ch会加倍，外层循环不变的情况下，ch也不发生改变

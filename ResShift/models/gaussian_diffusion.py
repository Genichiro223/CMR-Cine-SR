...
def _extract_into_tensor(arr, timesteps, broadcast_shape):  # arr是一维的数组，timesteps是给定的时间步，通过timesteps索引，在arr中提出对应的值，最后将
    """
    ...
    while len(res.shape) < len(broadcast_shape):
        res = res[..., None]  # 在最后新增一个维度，前面的是python的省略号对象，表示多个冒号，省略的维度
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        assert len(self.etas.shape) == 1, "etas must be 1-D"  # etas must be a 1-D array
        assert (self.etas > 0).all() and (self.etas <= 1).all()  # etas must be in (0, 1]
    ...
    def p_mean_variance(
        ...
        new_input = th.cat([x_t, y], dim=1).float()
        
        model_output = model(new_input, t)
    ...
    def p_sample_loop_progressive(
        ...
        z_sample = self.prior_sample(z_y, noise)  # 根据给定的低分辨率图像，从给定的先验分布中采样
    ...
    def decode_first_stage(self, z_sample, first_stage_model=None, is_training=False):
    ...
    def encode_first_stage(self, y, first_stage_model, up_sample=False, is_training=False):
        ori_dtype = y.dtype
        if up_sample:
    ...
    def training_losses(
        ...
        # y, x_start = forward_transform(y), forward_transform(x_start)  # 0-1转换到-1-1
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            ...
            # new_input = self._scale_input(z_t, t)
            # new_input = th.concat([new_input, z_y], dim=1)
            # model_output = model(new_input, t)  
            if np.isnan(model_output.detach().cpu().numpy()).any():
                print('model_output has nan!!!!!!!!!!!')
            target = {
        ...
        return terms, z_t, pred_zstart  # 默认设置是预测x_0 这里直接输出的就是模型的output

    def _scale_input(self, inputs, t):
        if self.normalize_input:  # true
            if self.latent_flag:  
                # the variance of latent code is around 1.0
                std = th.sqrt(_extract_into_tensor(self.etas, t, inputs.shape) * self.kappa**2 + 1).cuda()

...
def generalized_steps(initial_noise, seq, model, source_image, betas, **kwargs):
    with torch.no_grad():
        ...
        seq_next = [-1] + list(seq[:-1])  # [-1, 0, 10, 20, ..., 980]
        device = initial_noise.device
        x0_preds = []
        xs = [initial_noise]  # xs 记录了每个推理过程中的x_t
        for i, j in zip(reversed(seq), reversed(seq_next)):  #  i \in [990, 980, ..., 0]; j \in [980, 970, ..., 0, -1]
...
def ddpm_steps(initial_noise, seq, model, source_image, b, **kwargs):
    with torch.no_grad():
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            beta_t = 1 - at / atm1
            xt = xs[-1].to('cuda')
            y = torch.concatenate([source_image, xt], dim=1).to(device)
